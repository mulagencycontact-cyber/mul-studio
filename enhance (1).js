exports.handler = async function (event) {
  ...
  if (!REPLICATE_API_KEY) {
    return { statusCode: 500, body: JSON.stringify({ error: "REPLICATE_API_KEY is not set" }) };
  }
  ...
  try {
    ...
    if (mode === "placement") {
      ...
      // Use official model endpoint - no version hash needed
      ...
      try { predData = JSON.parse(predText); }
        ...
        return { statusCode: 500, body: JSON.stringify({ error: `Unexpected response: ${predText.slice(0, 300)}` }) };
      }
